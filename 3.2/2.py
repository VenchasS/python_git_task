...
if __name__ == '__main__':
    ...
    count_q = QueueP()
    count_prof_q = QueueP()
    salary_prof_q = QueueP()
    for year in range(2007, 2022 + 1):
        data = DataSet(f"files/{year}.csv", "Аналитик")
        proc = Process(target=data.start_multi, args=(salary_q, count_q, salary_prof_q, count_prof_q))
    ...
    print(f"Динамика уровня зарплат по годам: {dict(sorted(iterate(salary_q).items(), key = lambda x:x[0]))}")
    print(f"Динамика количества вакансий по годам: {dict(sorted(iterate(count_q).items(), key = lambda x:x[0]))}")
    print(f"Динамика уровня зарплат по годам для выбранной профессии: {dict(sorted(iterate(salary_prof_q).items(), key = lambda x:x[0]))}")
    print(f"Динамика количества вакансий по годам для выбранной профессии: {dict(sorted(iterate(count_prof_q).items(), key = lambda x:x[0]))}")
